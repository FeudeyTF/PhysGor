...
export function useLaws() {
  ...
  const deleteLaw = async (id: string) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unknown error deleting law"
      );
      return false;
    }
  };
  ...
  const createLaw = async (law: Omit<PhysicsLaw, "id">) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unknown error creating law"
      );
      return {
        success: false,
  ...
  const updateLaw = async (id: string, law: Omit<PhysicsLaw, "id">) => {
    try {
      ...
      if (result.success) {
        ...
        setLaws((prevLaws) =>
          prevLaws.map((l) => (l.id === id ? updatedLaw : l))
        );
      ...
      setError(
        err instanceof Error ? err.message : "Unknown error updating law"
      );
      return {
        success: false,
        error: err instanceof Error ? err.message : "Unknown error",
